...
export default class NextBatchCommand implements Command {
  public readonly slashCommand = new SlashCommandBuilder()
    .setName('next-batch')
    .setDescription('Adds the next batch of songs from a stored playlist to the queue.')
  ...
  public async handleAutocompleteInteraction(interaction: AutocompleteInteraction): Promise<void> {
    ...
    if (focusedValue.name === 'playlist') {
      ...
      const choices = storedPlaylists
        .filter(p => p.title.toLowerCase().includes(focusedValue.value.toLowerCase()))
  ...
  private async handleCommand(interaction: ChatInputCommandInteraction | Message, count?: number, playlistIdArg?: string): Promise<void> {
    ...
    if (playlistIdArg) {
      ...
      if (storedPlaylists.length === 0) {
        await (interaction instanceof Message ? interaction.channel.send(errorMsg('No playlist stored or it has expired. Play a playlist first.')) : interaction.reply({content: errorMsg('No playlist stored or it has expired. Play a playlist first.'), ephemeral: true}));
        return;
      } else if (storedPlaylists.length === 1) {
        targetPlaylistId = storedPlaylists[0];
      } else {
        await (interaction instanceof Message ? interaction.channel.send(errorMsg('Multiple playlists stored. Please specify which one using its ID or title (e.g., `]next-batch <count> <playlist_id>`).')) : interaction.reply({content: errorMsg('Multiple playlists stored. Please specify which one using the "playlist" option.'), ephemeral: true}));
        return;
      }
    }
    ...
    if (!storedPlaylist) {
      await (interaction instanceof Message ? interaction.channel.send(errorMsg('Specified playlist not found or it has expired.')) : interaction.reply({content: errorMsg('Specified playlist not found or it has expired.'), ephemeral: true}));
      return;
    }

    try {
      const addedSongs = player.addNextBatch(targetPlaylistId!, songsToAddCount);

      if (addedSongs.length === 0) {
        await (interaction instanceof Message ? interaction.channel.send(errorMsg('No more songs to add from this playlist.')) : interaction.reply({content: errorMsg('No more songs to add from this playlist.'), ephemeral: true}));
        return;
      }

      const remainingSongs = storedPlaylist.songs.length - storedPlaylist.addedCount;
      ...
      await (interaction instanceof Message ? interaction.channel.send(replyMsg) : interaction.reply(replyMsg));

    } catch (e: unknown) {
      await (interaction instanceof Message ? interaction.channel.send(errorMsg((e as Error).message)) : interaction.reply({content: errorMsg((e as Error).message), ephemeral: true}));
    }
  }
