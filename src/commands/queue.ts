...
export default class implements Command {
  public readonly slashCommand = new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand => subcommand
      ...
      .addStringOption(option => option
        .setName('page')
        .setDescription('page of queue to show (e.g., 1, top, current, last) [default: current]')
        .setRequired(false))
    ...
    .addSubcommand(subcommand => subcommand
      .setName('list')
      .setDescription('lists all saved playlists'))
    .addSubcommand(subcommand => subcommand
      .setName('delete')
  ...
  public async executePrefix(message: Message, args: string[]) {
    ...
    const validSubcommands = ['show', 'save', 'load', 'list', 'delete', 'export', 'import'];
    ...
    const mockInteraction = {
      ...
      reply: async (options: any) => {
        if (subcommand === 'import') {
          await message.reply({content: 'The `import` command is only supported via slash commands.', ephemeral: true});
          return;
        }
  ...
  private async executeShow(interaction: ChatInputCommandInteraction) {
    ...
    if (!pageOption || pageOption === 'current') {
      ...
      page = Math.ceil((queueSize + 1) / pageSize);
    } else {
      page = parseInt(pageOption, 10);
    }
  ...
  private async executeSave(interaction: ChatInputCommandInteraction) {
    ...
    const filePath = path.join(PLAYLISTS_DIR, `${guildId}-${sanitizedPlaylistName}.json`);

    try {
      await fs.access(filePath); // Check if file exists
      await interaction.reply({content: `Playlist "${playlistName}" already exists. Please choose a different name.`, ephemeral: true});
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        // File doesn't exist, so we can write it
        const playlistData = {
          name: playlistName,
          songs: queue.map(queuedSong => queuedSong.song),
        };

        await fs.writeFile(filePath, JSON.stringify(playlistData, null, 2));

        await interaction.reply({content: `Queue saved as "${playlistName}".`});
      } else {
        console.error(error);
  ...
  private async executeLoad(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      const songs: QueuedSong[] = playlistData.songs.map((song: SongMetadata) => ({
        ...song,
      ...
      await interaction.reply({content: `Playlist "${playlistName}" loaded and added to the queue.`});
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        await interaction.reply({content: `Playlist "${playlistName}" not found.`, ephemeral: true});
      } else {
        console.error(error);
        await interaction.reply({content: 'An error occurred while loading the playlist.', ephemeral: true});
      }
    }
  }
  ...
  private async executeList(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      const guildPlaylists = files.filter(file => file.startsWith(`${guildId}-`) && file.endsWith('.json'));

      if (guildPlaylists.length === 0) {
        await interaction.reply({content: 'No saved playlists found for this server.', ephemeral: true});
        return;
      }

      const playlistNames = await Promise.all(guildPlaylists.map(async file => {
      ...
      await interaction.reply({content: `**Saved Playlists:**\n- ${playlistNames.join('\n- ')}`});
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        await interaction.reply({content: 'No saved playlists found for this server.', ephemeral: true});
      } else {
        console.error(error);
        await interaction.reply({content: 'An error occurred while listing playlists.', ephemeral: true});
      }
    }
  }
  ...
  private async executeDelete(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      await interaction.reply({content: `Playlist "${playlistName}" deleted.`});
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        await interaction.reply({content: `Playlist "${playlistName}" not found.`, ephemeral: true});
      } else {
        console.error(error);
        await interaction.reply({content: 'An error occurred while deleting the playlist.', ephemeral: true});
      }
    }
  }
  ...
  private async executeExport(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        ...
        console.error(error);
        await interaction.reply({content: 'An error occurred while exporting the playlist.', ephemeral: true});
      }
    }
  }
  ...
  private async executeImport(interaction: ChatInputCommandInteraction) {
    ...
    if (!attachment) {
      await interaction.reply({content: 'Please attach a JSON file to import.', ephemeral: true});
      return;
    }

    if (!attachment.name?.endsWith('.json')) {
      await interaction.reply({content: 'The attached file must be a JSON file.', ephemeral: true});
      return;
    }
    ...
    try {
      ...
      if (!playlistData.songs || !Array.isArray(playlistData.songs)) {
        await interaction.reply({content: 'Invalid playlist file format. Missing 'songs' array.', ephemeral: true});
        return;
      }
      ...
      await interaction.reply({content: `Playlist "${playlistData.name ?? 'imported playlist'}" loaded and added to the queue.`});
    } catch (error) {
      console.error(error);
      await interaction.reply({content: 'An error occurred while importing the playlist. Please ensure it's a valid JSON file.', ephemeral: true});
    }
  }
}
