...
export default class PlayLikesCommand implements Command {
  ...
  public async execute(
    ...
    try {
      ...
      const favorites: SongMetadata[] = JSON.parse(
        fileContent,
      ) as SongMetadata[];

      if (favorites.length === 0) {
        await interaction.reply({
          content: "You have no favorited songs to play.",
          ephemeral: true,
        });
        return;
      }
      ...
      await interaction.reply({
        content: `Added ${favorites.length} favorited songs to the queue.`,
      });
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        ...
        console.error(error);
        await interaction.reply({
          content: "An error occurred while playing your favorited songs.",
          ephemeral: true,
        });
      }
    }
