...
export default class LikeCommand implements Command {
  public readonly slashCommand = new SlashCommandBuilder()
    .setName('like')
    .setDescription('saves the current song or a song from the queue as a favorite')
    .addStringOption(option => option
      .setName('position')
      .setDescription('position of the song in the queue (e.g., 1, current, next, +3)')
      .setRequired(false));
  ...
  public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!song) {
      await interaction.reply({content: 'No song found at that position.', ephemeral: true});
      return;
    }
    ...
    try {
      ...
      if (favorites.some(fav => fav.url === song!.url)) {
        await interaction.reply({content: `"${song.title}" is already in your favorites.`, ephemeral: true});
        return;
      }
      ...
      await interaction.reply({content: `Added "${song.title}" to your favorites!`});
    } catch (error) {
      console.error(error);
      await interaction.reply({content: 'An error occurred while saving your favorite song.', ephemeral: true});
    }
  }
